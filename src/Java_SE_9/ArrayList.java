...
public class ArrayList<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, Serializable
{
	...
	public boolean retainAll(Collection<?> c)
	{
		if(c == null)
		{
			throw new NullPointerException("You MUST provide a collection of values to retain in this ArrayList");
		}
		if(c.isEmpty())
		{
			this.size = 0;
	...
	public List<E> subList(int fromIndex, int toIndex)
	{
		if(fromIndex < 0 || toIndex > this.size)
		{
			throw new IndexOutOfBoundsException("Index " + fromIndex + " or Index " + toIndex + " is/are out of bounds");
		}
		ArrayList<E> list = new ArrayList<>();
		for(int i = fromIndex; i < toIndex; i++)
		{
	...
	public String toString()
	{
		...
		boolean isAString = this.array[0] instanceof String;
		boolean isACharacter = this.array[0] instanceof Character;
		buff.append("[");
		for(int i = 0; i < this.size; i++)
		{
			if(isAString)
